...
export default function BodegaInventario() {
  ...
  const itemsFiltrados = useMemo(() => {
    ...
    // selects
    if (f.instalacion !== "Todas las instalaciones") {
      out = out.filter(i => i.instalacion === f.instalacion);
    }
    if (f.categoria !== "Todas las categorías") {
      out = out.filter(i => i.categoria === f.categoria);
    }
    if (f.estado !== "Todos los estados") {
      out = out.filter(i => normalizaEstado(i.estado) === normalizaEstado(f.estado));
    }
    return out;
  }, [q, f, items]);
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {/* Estilos (se agregan inv2-* y mejoras visuales del modo instalación) */}
      <style>{`
        ...
        .inv2-blue{background:#4F46E5}
        .inv2-yellow{background:#F59E0B}
        .inv2-purple{background:#8B5CF6}
        .inv2-sku-text .inv2-prod{font-weight:700}
        .inv2-sku-text .inv2-code{font-size:12px;color:#6B7280}
        ...
        .inv2-empty{text-align:center;padding:32px 12px;color:#6B7280}
        .inv2-empty-illus{font-size:40px;margin-bottom:8px}
        ...
        @media (max-width: 640px){
          ...
          .inv2-thead > :nth-child(3),
          .inv2-row > :nth-child(3){display:none}
        }
      `}</style>
    </div>
  );
...
/* ===== Helpers visuales (sin dependencias) ===== */
...
function colorPorNombre(nombre=""){
  const n = nombre.toLowerCase();
  if (n.includes("casco")) return "inv2-blue";    // azul
  if (n.includes("zapato")) return "inv2-yellow"; // amarillo
  if (n.includes("guante")) return "inv2-purple"; // violeta
  return "inv2-blue";
}
