...
const DocumentosTab = ({
  ...
  // Estado: ver carpeta (pushpop con contenido)
  const [openFolder, setOpenFolder] = useState(null); // folder item

  // Estado: detalle doc
  const [openDoc, setOpenDoc] = useState(null); // doc item

  // Estado: renombrar (carpeta o doc)
  const [renameTarget, setRenameTarget] = useState(null); // {id, nombre}
  const [renameText, setRenameText] = useState("");

  // Estado: subir a carpeta
  const [uploadFolder, setUploadFolder] = useState(null); // folder item
  ...
  }, [showPush, renameTarget]);

  // Acciones básicas
  const openMenu = (e, it) => {
    const r = e.currentTarget.getBoundingClientRect();
    setMenu({ open:true, id: it.id, x: r.left, y: r.bottom + 6 });
  };
  ...
  const openFolderView = (folder) => { setOpenFolder(folder); closeMenu(); };
  const openDocView = (doc) => { setOpenDoc(doc); closeMenu(); };
  ...
  const removeItem = (it) => {
    // Si es carpeta, elimina también su contenido (hijos)
    if (isFolder(it)) {
  ...
  return (
    <div className="ed-card">
          ...
          {items.map((it) => (
            <tr key={it.id} style={{position:'relative'}}>
              <td style={{fontWeight:600}}>
                {isFolder(it) ? "📁" : "📄"} {it.nombre}
              </td>
              <td>{it.mod || "—"}</td>
              <td>{isFolder(it) ? "—" : (it.tam || "—")}</td>
              <td>
                <button className="ed-btn" onClick={(e)=>openMenu(e,it)}>⋯</button>
                {menu.open && menu.id === it.id && (
                  <div className="kebab-menu" style={{top: menu.y, left: menu.x}}>
                    {isFolder(it) ? (
                      <>
                        <button onClick={()=>openFolderView(it)}>📂 Ver</button>
                        <button onClick={()=>uploadIntoFolder(it)}>⬆️ Subir a esta carpeta</button>
                        <button onClick={()=>askRename(it)}>✏️ Renombrar</button>
                        <button className="danger" onClick={()=>removeItem(it)}>🗑️ Eliminar</button>
                      </>
                    ) : (
                      <>
                        <button onClick={()=>openDocView(it)}>👁️ Ver</button>
                        <button onClick={()=>fakeDownload(it.nombre)}>⬇️ Descargar</button>
                        <button onClick={()=>askRename(it)}>✏️ Renombrar</button>
                        <button className="danger" onClick={()=>removeItem(it)}>🗑️ Eliminar</button>
                      </>
                    )}
        ...
        <div className="pushpop-overlay" role="dialog" aria-modal="true" onClick={()=>setShowUpload(false)}>
          <div className="pushpop-card" role="document" onClick={(e)=>e.stopPropagation()}>
            ...
            <div className="pushpop-actions">
              ...
              <button
                className="ed-btn primary"
                onClick={()=>{ if (picked) onSubirArchivo?.(picked); setShowUpload(false); setPicked(null); }}
                disabled={!picked}
      ...
      {/* PushPop: Ver carpeta (contenido + subir) */}
      {openFolder && (
                ...
                {childrenOf(openFolder.id).map(doc => (
                  <li key={doc.id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:8, padding:'8px 0', borderTop:'1px solid #2a2a36'}}>
                    ...
                    <div style={{display:'flex', gap:6}}>
                      <button className="ed-btn" onClick={()=>openDocView(doc)}>Ver</button>
        ...
        <div className="pushpop-overlay" role="dialog" aria-modal="true" onClick={()=>setOpenDoc(null)}>
          <div className="pushpop-card" role="document" onClick={(e)=>e.stopPropagation()}>
            ...
            <div className="muted" style={{marginBottom:8}}>
              Últ. mod: {openDoc.mod || "—"} · Tamaño: {openDoc.tam || "—"}
            </div>
      ...
      <style>{`
        ...
        .kebab-menu button{
          text-align:left; background:transparent; border:none; color:#fff; padding:8px 10px; border-radius:8px; cursor:pointer;
        }
