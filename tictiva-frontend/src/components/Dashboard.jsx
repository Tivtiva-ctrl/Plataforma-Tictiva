...
function Drawer({ module, onClose }) {
  ...
  // Link visible (fallback) + navigate programático + cierre
  const onItemClick = (e, to) => {
    e.preventDefault();
        ...
        {MODULES.map((m) => (
          <article key={m.id} className={`card ${m.id}`}>
            <header className="card__head" onClick={() => setOpenModule(m)}>
              <div className="card__icon"><Icon name={m.icon} size={24} /></div>
