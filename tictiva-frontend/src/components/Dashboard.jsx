...
const Icon = ({ name, size = 24, className = "" }) => {
  ...
  switch (name) {
    case "lightbulb":
      return (
        <svg {...p}>
          <path d="M12 2a9 9 0 0 0-9 9c0 4.4 3.6 8 8 8v3a1 1 0 0 0 2 0v-3c4.4 0 8-3.6 8-8a9 9 0 0 0-9-9zM12 14a3 3 0 0 1-3-3h6a3 3 0 0 1-3 3z" />
        </svg>
      );
    case "id-card":
      return (
    ...
    case "pie-chart":
      return (
        <svg {...p}>
          <path d="M12 20v-8h8" />
          <path d="M12 20a8 8 0 1 0-8-8c0 .7.1 1.4.2 2" />
          <path d="M12 20c-4.4 0-8-3.6-8-8a8 8 0 0 1 8-8" />
        </svg>
      );
    case "bar-chart":
      return (
...
export default function Dashboard({ onLogout }) {
  ...
  // Cierra automáticamente el drawer cuando cambia la ruta
  useEffect(() => {
    if (openModule) setOpenModule(null);
  }, [location.pathname]); // no tocar deps
  ...
  const safeGo = (to) => {
    ...
    else alert("Este submódulo estará disponible pronto.");
  };
  ...
  return (
    <div className="dash">
        ...
        <div className="hero__content">
          ...
          <div className="tip">
            ...
            <div className="tip__text">
              ...
              <button className="linkBtn" onClick={() => safeGo(ROUTES.rrhhPermisos)}>
                Permisos
              </button>{" "}
              y{" "}
              <button className="linkBtn" onClick={() => safeGo(ROUTES.rrhhValidacionDT)}>
                Validación DT
              </button>{" "}
              en RR.HH.
            </div>
            <button className="tip__cta" onClick={() => safeGo(ROUTES.rrhhPermisos)}>
              Ir ahora →
            </button>
          </div>
        ...
        {MODULES.map((m) => (
          <article key={m.id} className={`card ${m.id}`}>
            ...
            <footer className="card__foot">
              <button className="card__open" onClick={() => setOpenModule(m)}>
                Abrir módulo →
              </button>
            </footer>
          </article>
      ...
      {openModule && (
        <Drawer module={openModule} onClose={() => setOpenModule(null)} />
      )}
    </div>
  );
}
