...
export default function ValidacionDT() {
  ...
  // ---- Demo/estado inicial (puedes cablearlo a json-server) ----
  // Empleados reales de tu listado
  ...
  // Ejemplos de “Límites de Jornada” con empleados reales
  const [limites] = useState([
    {
      rut: "14.567.890-1", // María Pérez Lagos
    ...
    {
      rut: "15.432.109-7", // Luis Soto Parra
  ...
  // Bitácora con nombres del listado
  const [auditoria] = useState([
    {
      id: 1,
      ts: "2024-07-30 10:15",
      usuario: "Laura RRHH",
      accion: "Justificación añadida",
      recurso: "Juan Díaz Morales",
      detalle: "Licencia Médica #123",
      ip: "10.1.2.3",
      obs: "",
    },
    {
      id: 2,
      ts: "2024-07-29 16:30",
      usuario: "Roberto AdminEmpresa",
      accion: "Edición de marca",
      recurso: "María Pérez Lagos",
      detalle: "Salida 18:02 → 18:05",
      ip: "10.1.2.4",
      obs: "Error en marcaje original.",
    },
    {
      id: 3,
      ts: "2024-07-28 09:00",
      usuario: "Sistema",
      accion: "Marca automática anulada",
      recurso: "Luis Soto Parra",
      detalle: "Detección de duplicado",
      ip: "-",
      obs: "",
    },
  ]);
  ...
  const [nuevo, setNuevo] = useState({
    ...
    horaHasta: "23:59",
    ambito:
      "Acceso completo a los registros de asistencia del período para fiscalización DT.",
  });

  // Estados calculados
  const estadoAsistencia =
    resumen.asistenciaCobertura7d >= 0.9
      ? "ok"
      : resumen.asistenciaCobertura7d >= 0.75
      ? "warn"
      : "danger";
  const estadoTrazabilidad =
    resumen.marcasConEvidencia >= 0.85
      ? "ok"
      : resumen.marcasConEvidencia >= 0.6
      ? "warn"
      : "danger";
  const estadoLimites =
    resumen.limitesCasosSemana >= 3
      ? "danger"
      : resumen.limitesCasosSemana >= 1
      ? "warn"
      : "ok";
  const estadoColacion =
    resumen.colacionOk >= 0.95 ? "ok" : resumen.colacionOk >= 0.8 ? "warn" : "danger";
  ...
  const auditoriaFiltrada = useMemo(() => {
    return auditoria.filter((a) => {
      const okQ = q
        ? `${a.usuario} ${a.recurso} ${a.accion}`.toLowerCase().includes(q.toLowerCase())
        : true;
  ...
  const statusPill = (s) =>
    s === "Generado" ? "pill pill-gen" : s === "Activo" ? "pill pill-act" : "pill pill-exp";
  ...
  const crearCodigo = async () => {
    ...
    const item = {
      ...
      fin: `${nuevo.hasta || new Date().toISOString().slice(0, 10)}T${
        nuevo.horaHasta || "23:59"
      }`,
      ambito: nuevo.ambito,
    ...
    // await fetch(`${API}/dt_codes`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(item)});
  };
    ...
    <div className="vdt-wrap">
      ...
      <div className="vdt-tabsbar">
        <div className="vdt-tabs">
          <button
            className={`vdt-tab ${tab === "panel" ? "is-active" : ""}`}
            onClick={() => setTab("panel")}
          >
            Panel de Cumplimiento
          </button>
          <button
            className={`vdt-tab ${tab === "accesos" ? "is-active" : ""}`}
          ...
          <div className="vdt-hero">
            ...
            <div>
              ...
              <p className="vdt-subtitle">
                Herramientas, checklists y reportes para asegurar la conformidad con los
                requerimientos de la Dirección del Trabajo de Chile.
              </p>
            </div>
          </div>
          ...
          <div className="vdt-panel">
            <div className="vdt-panel-head">
              ...
              <div>
                ...
                <p className="vdt-muted">
                  Estado general de los aspectos clave del cumplimiento legal y normativo.
                </p>
              </div>
              <div className="vdt-panel-actions">
                <button className="vdt-btn" onClick={() => alert("Export demo")}>
                  ⬇ Exportar
                </button>
              </div>
            </div>
            ...
            <div className="vdt-grid">
              ...
              <div
                className={statusClass(estadoAsistencia)}
                title="OK si ≥90% de jornadas con 2 marcas válidas (7 días)."
              >
                <div className="vdt-card-title">
                  <span>Registro de Asistencia</span>
                  <span className="vdt-state">
                    {estadoAsistencia === "ok" ? "✓" : estadoAsistencia === "warn" ? "ⓘ" : "⚠"}
                  </span>
                </div>
                <p>Sistema de marcación activo y funcional.</p>
                <div className="vdt-card-foot">
                  Cobertura últimos 7 días: {(resumen.asistenciaCobertura7d * 100).toFixed(0)}%
                </div>
              </div>

              {/* Trazabilidad */}
              <div
                className={statusClass(estadoTrazabilidad)}
                title="Porcentaje de marcas con evidencia (foto/GPS) + ediciones auditadas."
              >
                <div className="vdt-card-title">
                  <span>Trazabilidad de Marcas</span>
                  <span className="vdt-state">
                    {estadoTrazabilidad === "ok" ? "✓" : estadoTrazabilidad === "warn" ? "ⓘ" : "⚠"}
                  </span>
                </div>
                <p>Entradas, salidas y modificaciones registradas.</p>
                <div className="vdt-card-foot">
                  Con evidencia: {(resumen.marcasConEvidencia * 100).toFixed(0)}%
                </div>
              </div>
              ...
              <div className={statusClass(estadoLimites)}>
                <div className="vdt-card-title">
                  ...
                  <span className="vdt-state">
                    {estadoLimites === "ok" ? "✓" : estadoLimites === "warn" ? "ⓘ" : "⚠"}
                  </span>
                </div>
                ...
                <div className="vdt-list-mini">
                  {limites.slice(0, 2).map((l) => (
                    <div key={`${l.rut}-${l.fecha}`} className="vdt-mini-item">
                      <span
                        className="vdt-link"
                        onMouseEnter={() => setPanelLimites(true)}
                        onClick={() => setPanelLimites(true)}
                      >
                        {l.nombre} ({l.rut})
                      </span>
                      <button className="vdt-btn-link" onClick={() => setPanelLimites(true)}>
                        +info
                      </button>
                    </div>
                  ))}
              ...
              <div className={statusClass(estadoColacion)}>
                <div className="vdt-card-title">
                  ...
                  <span className="vdt-state">
                    {estadoColacion === "ok" ? "✓" : estadoColacion === "warn" ? "ⓘ" : "⚠"}
                  </span>
                </div>
                <p>Gestión de pausas habilitada.</p>
                <div className="vdt-card-foot">
                  Colación ≥30 min: {(resumen.colacionOk * 100).toFixed(0)}%
                </div>
              </div>
                ...
                <div className="vdt-list-mini">
                  {(resumen.docsFaltantes || []).slice(0, 2).map((d) => (
                    <div key={d.rut} className="vdt-mini-item">
                      <span
                        className="vdt-link"
                        onMouseEnter={() => setPanelDocs(d)}
                        onClick={() => setPanelDocs(d)}
                      >
                        {d.nombre} ({d.rut})
                      </span>
                      <Link className="vdt-btn-link" to={empleadoTabURL(d.rut, "documentos")}>
                        Ver Ficha
                      </Link>
                    </div>
                  ))}
              ...
              {/* Respaldos */}
              <div
                className={statusClass(estadoBackups)}
                title="Backups diarios firmados; retención 5 años."
              >
                <div className="vdt-card-title">
            ...
            <div className="vdt-note">
              Los indicadores son orientativos; ver detalles en cada módulo para revisión
              caso a caso.
            </div>
          </div>
          ...
          <div className="vdt-card full">
            <div className="vdt-panel-head">
              ...
              <div>
                ...
                <p className="vdt-muted">
                  Trazabilidad de ediciones, justificaciones y observaciones en los
            ...
            <div className="vdt-filters">
              <input
                className="vdt-input"
                placeholder="Buscar (Usuario/Empleado/Acción)"
                value={q}
            ...
            <div className="vdt-table">
              ...
              {!loading &&
                auditoriaFiltrada.map((a) => (
                  <div key={a.id} className="vdt-tr">
                    ...
                    <div className="vdt-td" title={a.detalle}>
                      {a.detalle}
                    </div>
              ...
              {!loading && auditoriaFiltrada.length === 0 && (
                <div className="vdt-empty">Aún no hay auditorías en este rango.</div>
              )}
            </div>
          </div>
            ...
            <>
              ...
              <aside className="vdt-sidepanel">
                <div className="vdt-panel-head">
                  ...
                  <button className="vdt-x" onClick={() => setPanelDocs(null)}>
                    ✖
                  </button>
                </div>
                <div className="vdt-panel-body">
                  ...
                  <ul className="vdt-ul">
                    {panelDocs.faltan.map((d, i) => (
                      <li key={i}>• {d}</li>
                    ))}
                  </ul>
                  <Link className="vdt-btn primary" to={empleadoTabURL(panelDocs.rut, "documentos")}>
                    Ir a Documentos
                  </Link>
                </div>
              </aside>
            ...
            <>
              ...
              <aside className="vdt-sidepanel">
                <div className="vdt-panel-head">
                  ...
                  <button className="vdt-x" onClick={() => setPanelLimites(false)}>
                    ✖
                  </button>
                </div>
                <div className="vdt-panel-body">
                  <div className="vdt-filters">
                    <select className="vdt-input" defaultValue="Semana">
                      {["Día", "Semana", "Mes"].map((v) => (
                        <option key={v}>{v}</option>
                      ))}
                    </select>
                    <select className="vdt-input" defaultValue="Empleado">
                      {["Empleado", "Puesto", "Centro"].map((v) => (
                        <option key={v}>{v}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 12 }}>
                    <button className="vdt-btn" onClick={() => alert("Ir a Asistencia")}>
                      Ir a Asistencia
                    </button>
                    <button className="vdt-btn" onClick={() => alert("Export demo")}>
                      ⬇ Exportar
        ...
        <>
          <div className="vdt-hero">
            ...
            <div>
              ...
              <p className="vdt-subtitle">
                Administra códigos de acceso temporal para fiscalizadores de la Dirección del Trabajo.
              </p>
            </div>
          ...
          <div className="vdt-card full">
            <div className="vdt-table">
              <div className="vdt-tr vdt-th">
                <div>Código</div>
                <div>Nombre Fiscalizador</div>
                <div>Vigencia</div>
                <div>Ámbito del Acceso</div>
                <div>Creado Por</div>
              ...
              {codes.map((c) => (
                <div key={c.id} className="vdt-tr">
                  ...
                  <div className="vdt-td">
                    {c.inicio.replace("T", " ")} — {c.fin.replace("T", " ")}
                  </div>
                  <div className="vdt-td">{c.ambito}</div>
                  <div className="vdt-td">{c.creadoPor}</div>
                  <div className="vdt-td">{c.creadoTs}</div>
                  <div className="vdt-td">
                    <span className={statusPill(codeStatus(c))}>{codeStatus(c)}</span>
                  </div>
                  <div className="vdt-td actions">
                    ...
                    <button className="vdt-icon" title="Ver" onClick={() => alert(`Código: ${c.code}`)}>
                      👁️
                    </button>
                    <button className="vdt-icon" title="Regenerar" onClick={() => alert("Regenerar (demo)")}>
                      🔁
                    </button>
                    <button
                      className="vdt-icon danger"
                      title="Eliminar"
                      onClick={() => setCodes((p) => p.filter((x) => x.id !== c.id))}
                    >
                      🗑️
                    </button>
                  </div>
            ...
            <>
              ...
              <div className="vdt-modal">
                <div className="vdt-modal-head">
                  ...
                  <button className="vdt-x" onClick={() => setModalOpen(false)}>
                    ✖
                  </button>
                </div>
                <div className="vdt-modal-body">
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <input
                      className="vdt-input"
                      value={nuevo.code}
                      onChange={(e) => setNuevo({ ...nuevo, code: e.target.value.toUpperCase() })}
                    />
                    <button className="vdt-btn" onClick={() => setNuevo({ ...nuevo, code: genCode() })}>
                      Regenerar
                    </button>
                  </div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    value={nuevo.ambito}
                    onChange={(e) => setNuevo({ ...nuevo, ambito: e.target.value })}
                  />
                </div>
                <div className="vdt-modal-foot">
                  <button className="vdt-btn" onClick={() => setModalOpen(false)}>
                    Cancelar
                  </button>
                  <button className="vdt-btn primary" onClick={crearCodigo}>
