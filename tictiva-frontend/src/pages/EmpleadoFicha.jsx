    ...
    const load = async () => {
      ...
      try {
        ...
        if (likelyRut(ref)) q = q.eq("rut", ref);
          ...
          // Si viene un número (ej: "3") desde alguna ruta vieja, intenta por RUT primero.
          q = q.or(`rut.eq.${ref},id.eq.${ref}`);
        }
  ...
  if (loading) {
    return (
      <div className="ef-page">
        <div className="ef-wrap"><div className="ef-card p20">Cargando ficha…</div></div>
      </div>
    );
  }
      ...
      <div className="ef-page">
        <div className="ef-wrap">
          <div className="ef-card p20" style={{marginTop:16}}>
            <h3 className="ef-title-sm">No encontramos la ficha para RUT: <strong>{ref}</strong></h3>
            <button className="lf-btn lf-btn-primary mt12" onClick={() => navigate("/rrhh/listado-fichas")}>
              Volver al Listado
            </button>
          </div>
        </div>
  ...
  return (
    <div className="ef-page">
      <div className="ef-wrap">
        ...
        {/* Layout principal: izquierda + derecha (derecha se oculta en Asistencia/Historial) */}
        <div className={`ef-layout ${sidebarHidden ? "full" : ""}`}>
          ...
          {!sidebarHidden && (
            <div className="ef-side">
              <div className="ef-card p16">
                <h3 className="ef-side-title">Información Rápida</h3>
                <ul className="ef-info">
                  <li><span>RUT:</span><strong>{emp.rut || "—"}</strong></li>
                  <li><span>Género:</span><strong>{emp.genero || "—"}</strong></li>
                  <li><span>Estado:</span><strong>{emp.activo ? "Activo" : "Inactivo"}</strong></li>
                </ul>
              </div>

              <div className="ef-card p16">
                <h3 className="ef-side-title">Rendimiento</h3>
                <div className="ef-meter"><span style={{width:"92%"}} /></div>
                <div className="ef-meter"><span style={{width:"96%"}} /></div>
                <div className="ef-meter purple"><span style={{width:"88%"}} /></div>
              </div>
            </div>
          )}
        </div>
...
/* ======== Secciones (puedes reemplazar por tus componentes reales) ======== */
...
function Contractuales() {
  return (
    <div className="ef-card p20">
      ...
      <div className="ef-hint">Aquí va el detalle de contrato, tipo, vigencia, anexos, etc.</div>
    </div>
  );
}
...
function Asistencia() {
  return (
    <div className="ef-card p20">
      ...
      <div className="ef-hint">Aquí va el resumen de marcas, calendario, cumplimiento DT, etc.</div>
    </div>
  );
}
