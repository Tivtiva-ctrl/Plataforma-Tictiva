...
export default function ListadoFichas() {
  ...
  // Filtro KPI (si no tienes lógica aún, devolvemos lo mismo)
  const listFiltered = useMemo(() => {
    // TODO: aplica tu switch por estados/KPI si corresponde
    return listBySearch;
  ...
  return (
    <div className="dashboard-bg" style={{ padding: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input
            ...
            style={{ minWidth: 260 }}
          />
          <button className="vdt-btn" onClick={() => setOpen(true)}>+ Nuevo</button>
        </div>
      </div>

      {/* KPIs (placeholder simple) */}
      <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
        <button className={`vdt-btn ${kpiFilter === "todos" ? "primary" : ""}`} onClick={() => setKpiFilter("todos")}>
          Todos ({total})
        </button>
        {/* Agrega más KPIs según tu lógica */}
      </div>
          ...
          listFiltered.map((e) => {
            ...
            return (
              ...
              >
                <div>
                  <div style={{
                    width: 40, height: 40, borderRadius: 999, background: "#E5EDFF",
      ...
      {open && (
        <>
          {/* Tu contenido de modal para crear empleado */}
        </>
      )}
