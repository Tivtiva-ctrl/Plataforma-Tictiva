...
const VolverAtras = () => {
  ...
  return <button onClick={() => navigate(-1)} className="volver-btn">&larr; Volver</button>;
};
...
const PersonalesTab = ({ empleado, modoEdicion, onChange }) => {
    ...
    const fields = [ { key: "nombre", label: "Nombre Completo", type: "text" }, { key: "rut", label: "Cédula", type: "text", disabled: true }, { key: "fechaNacimiento", label: "Fecha de Nacimiento", type: "date" }, { key: "correo", label: "Email", type: "email" }, { key: "telefono", label: "Teléfono", type: "tel" }, { key: "direccion", label: "Dirección", type: "text" }, { key: "estadoCivil", label: "Estado Civil", type: "text" }];
    return (
        ...
        <div className="ed-kv">
          {fields.map(f => (
            <div className="ed-kv-row" key={f.key}>
              ...
              {modoEdicion ? (<input type={f.type} value={f.key === 'fechaNacimiento' ? cumpleISO.slice(0,10) : (empleado[f.key] || "")} onChange={(e) => onChange(f.key, e.target.value)} disabled={f.disabled} />) : (<span className="ed-kv-value">{f.key === 'fechaNacimiento' ? fmtFechaLarga(cumpleISO) : (empleado[f.key] || "—")}</span>)}
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="ed-card">
      ...
      <div className="ed-2col">
        {fields.map(f => (
          <div className="ed-kv-row" key={f.key}>
            <span className="ed-kv-label">{f.label}:</span>
            {modoEdicion ? (<input type={f.type || "text"} value={datos[f.key] || ""} onChange={(e) => handleFieldChange(f.key, e.target.value)} />) : (<span className="ed-kv-value">{f.key === 'sueldoBase' && datos[f.key] ? `$${Number(datos[f.key]).toLocaleString('es-CL')}` : (datos[f.key] || "N/D")}</span>)}
          </div>
        ))}
      </div>
    </div>
...
const PrevisionTab = ({ empleado, modoEdicion, onChange }) => {
    ...
    return (
      <div className="ed-card">
        <h3 className="ed-card-title">Datos Previsionales</h3><div className="ed-2col">{fields.map(f => (<div className="ed-kv-row" key={f.key}><span className="ed-kv-label">{f.label}:</span>{modoEdicion ? ( <input type="text" value={pv[f.key] || ""} onChange={(e) => handleFieldChange(f.key, e.target.value)} /> ) : ( <span className="ed-kv-value">{pv[f.key] || "N/D"}</span> )}</div>))}</div>
      </div>
    );
};
const BancariosTab = ({ empleado, modoEdicion, onChange }) => {
    ...
    return (
      <div className="ed-card">
        <h3 className="ed-card-title">Datos Bancarios</h3><div className="ed-2col">{fields.map(f => (<div className="ed-kv-row" key={f.key}><span className="ed-kv-label">{f.label}:</span>{modoEdicion ? ( <input type="text" value={b[f.key] || ""} onChange={(e) => handleFieldChange(f.key, e.target.value)} /> ) : ( <span className="ed-kv-value">{b[f.key] || "N/D"}</span> )}</div>))}</div>
      </div>
    );
};
function HojaDeVida({ empleado, modoEdicion, onChange }) {
  ...
  const handleHvChange = (field, value, section = null, index = -1) => {
    ...
    if (section) { (index > -1) ? newHv[section][index][field] = value : newHv[section][field] = value; } else { newHv[field] = value; }
    onChange('hojaVida', newHv);
  };
  return (
    <div className="ed-card">
      <h3 className="ed-card-title">Hoja de Vida y Ficha Médica</h3>
      <div className="hv-card"><h4 className="hv-title">Alerta Médica</h4>{modoEdicion ? (<input type="text" value={hv.alertaMedica || ""} onChange={(e) => handleHvChange('alertaMedica', e.target.value)}/>) : (<div className="hv-alert">{hv.alertaMedica || "Sin alertas"}</div>)}</div>
        ...
        <div className="hv-grid2">
          {(hv.emergencia || []).slice(0, 2).map((c, idx) => (
            <div key={idx} className="hv-contact">
              {modoEdicion ? (<div className="form-stack"><input placeholder="Nombre" value={c.nombre || ""} onChange={e => handleHvChange('nombre', e.target.value, 'emergencia', idx)} /><input placeholder="Relación" value={c.relacion || ""} onChange={e => handleHvChange('relacion', e.target.value, 'emergencia', idx)} /><input placeholder="Teléfono" value={c.telefono || ""} onChange={e => handleHvChange('telefono', e.target.value, 'emergencia', idx)} /></div>) : (<><div className="hv-strong">{c.nombre || "N/D"}</div><div className="hv-muted">{c.relacion || ""}</div><div className="hv-row"><span className="hv-label">Teléfono:</span><span className="hv-val">{c.telefono || "—"}</span></div></>)}
            </div>
          ))}
        </div>
      </div>
...
export default function EmpleadoDetalle() {
  ...
  const [notFound, setNotFound] = useState(false); // Estado para manejar si no se encuentra
  ...
  // ✅ ¡ESTRUCTURA CORREGIDA! ESTOS RETURNS ESTÁN AHORA DENTRO DE LA FUNCIÓN
  if (notFound) {
    return (
      <div className="ed-wrap">
        <VolverAtras />
        <div className="ed-card">Empleado no encontrado.</div>
      </div>
    );
  }
  ...
  if (!empleado) {
    return (
      <div className="ed-wrap">
        <div className="ed-card">Cargando...</div>
      </div>
    );
  }
  ...
  return (
    <div className="ed-wrap">
      ...
      <div className="ed-card ed-head">
        ...
        {modoEdicion ? (<><button className="ed-btn" onClick={cancelarEdicion}>Cancelar</button><button className="ed-btn primary" onClick={guardarEmpleado}>Guardar Cambios</button></>) : (<button className="ed-btn" onClick={() => setModoEdicion(true)}>Editar Ficha</button>)}
        </div>
      </div>
      ...
      <div className="ed-grid">
        ...
        <aside className="ed-right"><div className="ed-card"><h4 className="ed-card-title">Información Rápida</h4><p className="placeholder-text">Resumen...</p></div></aside>
      </div>
      ...
      <style>{`
        ...
        input[type="text"], input[type="email"], input[type="tel"], input[type="date"], input[type="number"] { border: 1px solid #D1D5DB; border-radius: 0.5rem; padding: 0.5rem 0.75rem; font-size: 1rem; font-family: inherit; width: 100%; box-sizing: border-box; }
